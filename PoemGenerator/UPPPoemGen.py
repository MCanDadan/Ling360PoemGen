...
def normal_poem_trigrams():
    ...
    poem_trigrams = [((final_corpus[i], final_corpus[i + 1]), final_corpus[i + 2]) for i in
                     range(len(final_corpus) - 2)]
...
def normal_line_maker():
    ...
    no_initial = ["da", "de", "misin", "mısın", "müsün", "musun", "mi", "mu", "mı", "mü", "ve",
                  "ile"]  # expand the list
...
def reverse_line_maker(reverse_corner):
    ...
    for i in range(a):
        ...
        reverse_word_probabilities = [reverse_poem_trigram_pbs[reverse_current_word].prob(word) for word in
                                      reverse_probable_words]
